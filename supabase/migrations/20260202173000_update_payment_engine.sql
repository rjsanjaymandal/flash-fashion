-- Update Master Payment Engine to v5.1 (Includes Coupon Logic)
...
BEGIN
  ...
  -- 7. Process Coupon Redemption (Atomic)
  DECLARE
    v_coupon_code TEXT;
  BEGIN
    SELECT coupon_code INTO v_coupon_code FROM orders WHERE id = p_order_id;
    IF v_coupon_code IS NOT NULL THEN
       PERFORM redeem_coupon(v_coupon_code, v_user_id);
    END IF;
  EXCEPTION WHEN OTHERS THEN
    -- Log but do not fail the payment for a coupon error
    INSERT INTO system_logs (component, message, severity, metadata)
    VALUES ('PAYMENT_COUPON_FAIL', 'Failed to redeem coupon for order ' || p_order_id, 'WARN', jsonb_build_object('error', SQLERRM));
  END;
