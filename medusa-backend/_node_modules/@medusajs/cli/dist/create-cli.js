...
const index_1 = require("./codemods/index");
...
function buildLocalCommands(cli, isLocalProject) {
    ...
    function resolveLocalCommand(command) {
        if (!isLocalProject && command !== "new") {
            console.error(`The "${command}" command must be run inside a Medusa project. Make sure you are in the root directory of a Medusa project and try again.`);
            process.exit(1);
        }
        try {
    ...
    cli
        ...
        handler: handlerP(getCommandHandler("plugin/build", async (args, cmd) => {
            process.env.NODE_ENV = process.env.NODE_ENV || `development`;
            cmd(args);
        ...
        handler: handlerP(getCommandHandler("plugin/develop", async (args, cmd) => {
            process.env.NODE_ENV = process.env.NODE_ENV || `development`;
            cmd(args);
            return new Promise(() => { });
        })),
        ...
        handler: handlerP(getCommandHandler("plugin/publish", async (args, cmd) => {
            process.env.NODE_ENV = process.env.NODE_ENV || `development`;
            cmd(args);
            return new Promise(() => { });
        })),
        ...
        handler: handlerP(getCommandHandler("plugin/add", async (args, cmd) => {
            process.env.NODE_ENV = process.env.NODE_ENV || `development`;
            cmd(args);
            return new Promise(() => { });
        })),
        ...
        handler: handlerP(async ({ codemodName, dryRun }) => {
            const codemod = (0, index_1.getCodemod)(codemodName);
            if (!codemod) {
                const available = (0, index_1.listCodemods)();
                reporter_1.default.error(`Unknown codemod: ${codemodName}`);
                reporter_1.default.info(`\nAvailable codemods:\n${available
                    .map((n) => `  - ${n}`)
                    .join("\n")}`);
                process.exit(1);
            }
            reporter_1.default.info(`Running codemod: ${codemod.name}`);
            reporter_1.default.info(codemod.description);
            if (dryRun) {
                reporter_1.default.info(`\n  DRY RUN MODE - No files will be modified\n`);
            }
            const result = await codemod.run({ dryRun });
            reporter_1.default.info(`\n Summary:`);
            reporter_1.default.info(`   Files scanned: ${result.filesScanned}`);
            reporter_1.default.info(`   Files modified: ${result.filesModified}`);
            reporter_1.default.info(`   Errors: ${result.errors}`);
            if (dryRun && result.filesModified > 0) {
                ...
                reporter_1.default.info(`\n Next steps:`);
                reporter_1.default.info(`   1. Review changes: git diff`);
                reporter_1.default.info(`   2. Run tests to verify`);
                reporter_1.default.info(`   3. Commit if satisfied`);
            }
            else {
        ...
        handler: handlerP(getCommandHandler(`develop`, async (args, cmd) => {
            process.env.NODE_ENV = process.env.NODE_ENV || `development`;
            cmd(args);
        ...
        handler: handlerP(getCommandHandler(`start`, async (args, cmd) => {
        ...
        handler: handlerP(getCommandHandler(`build`, async (args, cmd) => {
            process.env.NODE_ENV = process.env.NODE_ENV || `development`;
            cmd(args);
            return new Promise((resolve) => { });
        })),
        ...
        handler: handlerP(getCommandHandler(`user`, async (args, cmd) => {
            cmd(args);
            // Return an empty promise to prevent handlerP from exiting early.
            // The development server shouldn't ever exit until the user directly
            // kills it so this is fine.
        ...
        handler: handlerP(getCommandHandler(`exec`, async (args, cmd) => {
            cmd(args);
            // Return an empty promise to prevent handlerP from exiting early.
            // The development server shouldn't ever exit until the user directly
            // kills it so this is fine.
