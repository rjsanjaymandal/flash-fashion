...
const generateEntityColumns = (entity, entityMapping) => {
  ...
  `;
    ...
    const additionalTypes = ADDITIONAL_ENTITY_TYPES[entity] ?? [];
    additionalTypes.forEach((typeName) => {
        const additionalType = schemaTypeMap[typeName];
        if (!additionalType) {
            return;
        }
        const additionalFields = (0, utils_1.graphqlSchemaToFields)(schemaTypeMap, typeName, []);
    ...
    const directColumns = directFields.map((fieldName) => {
        ...
        const entityOrders = exports.DEFAULT_COLUMN_ORDERS[entity] || {};
    ...
    if (finalRelations.size > 0) {
        for (const [relationName, relatedTypeName] of finalRelations) {
            ...
            limitedFields.forEach((fieldName) => {
                ...
                const entityOrders = exports.DEFAULT_COLUMN_ORDERS[entity] || {};
                const defaultOrder = entityOrders[fieldPath] || (isDefaultField ? 700 : 850);
    ...
    if (entityMapping.computedColumns) {
        for (const [columnId, columnConfig] of Object.entries(entityMapping.computedColumns)) {
            ...
            const entityOrders = exports.DEFAULT_COLUMN_ORDERS[entity] || {};
            const defaultOrder = entityOrders[columnId] || (isDefaultField ? 600 : 850);
