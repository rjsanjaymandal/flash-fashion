    ...
    static validateCreateRequest(stores) {
        for (const store of stores) {
            if (store.supported_currencies?.length) {
                StoreModuleService.validateUnique(store.supported_currencies.map((currency) => currency.currency_code), "currency");
            ...
            if (store.supported_locales?.length) {
                StoreModuleService.validateUnique(store.supported_locales.map((locale) => locale.locale_code), "locale");
            }
        }
    }
...
StoreModuleService.validateUnique = (items, fieldName) => {
    const duplicates = (0, utils_1.getDuplicates)(items);
    if (duplicates.length) {
        throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, `Duplicate ${fieldName} codes: ${duplicates.join(", ")}`);
    }
};
