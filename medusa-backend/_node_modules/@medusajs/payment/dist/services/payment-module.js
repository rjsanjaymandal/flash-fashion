...
class PaymentModuleService extends utils_1.ModulesSdkUtils.MedusaService(generateMethodForModels) {
    ...
    async capturePayment(data, sharedContext = {}) {
        ...
        const payment = await this.paymentService_.retrieve(data_.payment_id, {
            select: [
                "id",
                "data",
                "provider_id",
        ...
        const { isFullyCaptured, capture } = await this.capturePayment_(data_, payment, sharedContext);
        try {
            await this.capturePaymentFromProvider_(payment, capture, { isFullyCaptured, isCaptured }, sharedContext);
        }
        catch (error) {
            if (capture?.id) {
    ...
    async capturePaymentFromProvider_(payment, capture, options = {}, sharedContext = {}) {
