...
const mikro_orm_cli_1 = require("../deps/mikro-orm-cli");
const mikro_orm_core_1 = require("../deps/mikro-orm-core");
const path_1 = require("path");
/**
 * Monkey patching the TSNode registration of Mikro ORM to not use
 * hardcoded module system with TypeScript.
 */
mikro_orm_core_1.ConfigurationLoader.registerTsNode = function (configPath = "tsconfig.json") {
...
require("@jercle/yargonaut")
    ...
    .errorsStyle("red");
(async () => {
    const argv = await mikro_orm_cli_1.CLIConfigurator.configure();
    const args = await argv.parse(process.argv.slice(2));
    if (args._.length === 0) {
        mikro_orm_cli_1.CLIHelper.showHelp();
    }
})();
