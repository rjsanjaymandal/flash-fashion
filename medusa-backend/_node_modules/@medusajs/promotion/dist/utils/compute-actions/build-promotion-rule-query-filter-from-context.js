...
exports.buildPromotionRuleQueryFilterFromContext = buildPromotionRuleQueryFilterFromContext;
...
async function buildPromotionRuleQueryFilterFromContext(context, sharedContext) {
    const { items = [], shipping_methods: shippingMethods = [], ...restContext } = context;
    let flattenItemsPropsValuesArray = (0, utils_1.flattenObjectToKeyValuePairs)(items);
    flattenItemsPropsValuesArray = Object.fromEntries(Object.entries(flattenItemsPropsValuesArray).map(([k, v]) => [
        `items.${k}`,
        v,
    ]));
    let flattenShippingMethodsPropsValuesArray = (0, utils_1.flattenObjectToKeyValuePairs)(shippingMethods);
    flattenShippingMethodsPropsValuesArray = Object.fromEntries(Object.entries(flattenShippingMethodsPropsValuesArray).map(([k, v]) => [
        `shipping_methods.${k}`,
        v,
    ]));
    const flattenRestContextPropsValuesArray = (0, utils_1.flattenObjectToKeyValuePairs)(restContext);
    ...
    if (numberOfAttributes > 10) {
      ...
      `);
        const dbAvailableAttributes = new Set(rows.map(({ attribute }) => attribute));
    ...
    attributeValueMap.forEach((valueSet, attribute) => {
        ...
        if (stringValues.length) {
            sqlConditions.push(`(pr.attribute = ${escapedAttribute} AND pr.operator IN ('in', 'eq') AND pr.id NOT IN (
        ...
        if (numericValues.length) {
            ...
            sqlConditions.push(`(pr.attribute = ${escapedAttribute} AND pr.operator = 'gt' AND CAST(prv.value AS DECIMAL) >= ${maxValue})`);
            // For gte - rule is unsatisfiable if rule_value > context_max_value
            sqlConditions.push(`(pr.attribute = ${escapedAttribute} AND pr.operator = 'gte' AND prv.value NOT IN (${stringValues}) AND CAST(prv.value AS DECIMAL) > ${maxValue})`);
            // For lt - rule is unsatisfiable if rule_value <= context_min_value
            sqlConditions.push(`(pr.attribute = ${escapedAttribute} AND pr.operator = 'lt' AND CAST(prv.value AS DECIMAL) <= ${minValue})`);
            // For lte - rule is unsatisfiable if rule_value < context_min_value
            sqlConditions.push(`(pr.attribute = ${escapedAttribute} AND pr.operator = 'lte' AND prv.value NOT IN (${stringValues}) AND CAST(prv.value AS DECIMAL) < ${minValue})`);
        }
    });
  ...
  `.trim();
    return {
        [(0, postgresql_1.raw)((alias) => antiJoinSubquery(alias))]: true,
    };
}
//# sourceMappingURL=build-promotion-rule-query-filter-from-context.js.map