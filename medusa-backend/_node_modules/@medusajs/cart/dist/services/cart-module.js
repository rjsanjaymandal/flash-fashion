...
class CartModuleService extends utils_1.ModulesSdkUtils.MedusaService(generateMethodForModels) {
    ...
    async createCarts(data, sharedContext = {}) {
        ...
        const result = await this.baseRepository_.serialize(carts);
        return (Array.isArray(data) ? result : result[0]);
    }
    async createCarts_(data, sharedContext = {}) {
        const cartsWithItems = data.map(({ items, ...cart }) => {
            const cartId = (0, utils_1.generateEntityId)(cart.id, "cart");
            return {
                cart: { ...cart, id: cartId },
                items: items || [],
            };
        ...
        const createdCarts = await this.cartService_.create(cartsWithItems.map((c) => c.cart), sharedContext);
        const lineItemsToCreate = [];
        for (const { cart, items } of cartsWithItems) {
            if (items.length) {
                lineItemsToCreate.push(...items.map((item) => ({
                    ...item,
                    cart_id: cart.id,
                })));
            }
        ...
        const fullCarts = await this.cartService_.list({ id: createdCarts.map((c) => c.id) }, {
            relations: ["shipping_address", "billing_address"],
        }, sharedContext);
        // Return in the same input order
        const orderedInputId = cartsWithItems.map((c) => c.cart.id);
        return orderedInputId.map((id) => fullCarts.find((c) => c.id === id));
    }
    // @ts-expect-error
