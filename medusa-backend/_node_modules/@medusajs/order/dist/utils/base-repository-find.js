...
const _1 = require(".");
function ensureOrderItemFieldsSelection(config, isRelatedEntity) {
    const populate = config.options?.populate ?? [];
    const fields = config.options?.fields ?? [];
    const hasItemsItemPopulate = populate.some((p) => p === "items.item" ||
    ...
    const hasOrderItemFields = fields.some((field) => {
        ...
        if (field.startsWith("order.items.") &&
...
function setFindMethods(klass, entity) {
    klass.prototype.find = async function find(options, context) {
        ...
        if (config.options.populate.includes("items.item.adjustments")) {
            ...
            config.options.populate.splice(config.options.populate.indexOf("items.item.adjustments"), 1);
            ...
            if (config.options.fields?.some((f) => f.includes("items.item."))) {
                config.options.fields.push(isRelatedEntity ? "order.items.version" : "items.version");
            }
        }
        ...
        if (strategy === core_1.LoadStrategy.SELECT_IN) {
            ...
            utils_1.MikroOrmBaseRepository.compensateRelationFieldsSelectionFromLoadStrategy({
                findOptions: config,
            });
        }
    ...
    klass.prototype.findAndCount = async function findAndCount(findOptions = { where: {} }, context = {}) {
        ...
        const [result, count] = await manager.findAndCount(this.entity, config.where, config.options);
        if (loadAdjustments) {
            const orders = !isRelatedEntity
                ? [...result]
                : [...result].map((r) => r.order).filter(Boolean);
...
async function loadItemAdjustments(manager, orders) {
    ...
    const itemsIdMap = new Map(items.map((i) => [i.item.id, i.item]));
    if (!items.length) {
        return;
    }
    ...
    const adjustments = await manager.find(_models_1.OrderLineItemAdjustment, {
        $or: params,
    });
