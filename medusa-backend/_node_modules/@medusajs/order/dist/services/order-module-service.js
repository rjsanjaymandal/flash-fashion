...
class OrderModuleService extends utils_1.ModulesSdkUtils.MedusaService(generateMethodForModels) {
    ...
    async createOrders_(data, sharedContext = {}) {
        ...
        for (const data_ of data) {
            const { items, shipping_methods, credit_lines, shipping_address, billing_address, ...order } = data_;
            const ord = order;
            ...
            ord.custom_display_id = await this.generateCustomDisplayId_.bind(this)(data_, sharedContext);
    ...
    async deleteOrders(orderIds, sharedContext = {}) {
        ...
        if (deletions.length) {
            await (0, utils_1.promiseAll)(deletions);
        }
    ...
    async previewOrderChange(orderId, sharedContext = {}) {
        ...
        const { itemsToUpsert, shippingMethodsToUpsert, calculatedOrders, lineItemAdjustmentsToCreate, } = await (0, utils_2.applyChangesToOrder)([order], { [order.id]: sortedActions }, { addActionReferenceToObject: true });
        const calculated = calculatedOrders[order.id];
        await this.includeTaxLinesAndAdjustmentsToPreview(calculated.order, itemsToUpsert, shippingMethodsToUpsert, lineItemAdjustmentsToCreate, // this will add "virtual" adjustments for the preview version but no actual adjustments will be created in the DB
        sharedContext);
    ...
    async includeTaxLinesAndAdjustmentsToPreview(order, itemsToUpsert, shippingMethodsToUpsert, lineItemAdjustmentsToCreate, sharedContext = {}) {
        const addedItems = {};
        ...
        if (Object.keys(addedItems).length > 0) {
            ...
            order.items.forEach((item, idx) => {
                ...
                const adjustments = lineItemAdjustmentsToCreate.filter((d) => d.item_id === newItem.item_id);
    ...
    async applyOrderChanges_(changeActions, sharedContext = {}) {
        ...
        const { itemsToUpsert, shippingMethodsToUpsert, summariesToUpsert, orderToUpdate, creditLinesToUpsert, lineItemAdjustmentsToCreate, } = await (0, utils_2.applyChangesToOrder)(orders, actionsMap, {
            addActionReferenceToObject: true,
            includeTaxLinesAndAdjustmentsToPreview: async (...args) => {
                args.push(sharedContext);
                return await this.includeTaxLinesAndAdjustmentsToPreview.apply(this, args);
            },
        });
        const [_orderUpdate, _orderChangeActionUpdate, orderItems, _orderSummaryUpdate, orderShippingMethods, orderCreditLines,] = await (0, utils_1.promiseAll)([
            ...
            lineItemAdjustmentsToCreate.length
                ...
                lineItemAdjustmentsToCreate, sharedContext)
                : null,
        ]);
        return {
            items: orderItems ?? [],
