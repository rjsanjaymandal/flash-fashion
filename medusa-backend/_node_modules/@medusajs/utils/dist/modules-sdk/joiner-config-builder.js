...
exports.buildIdPrefixToEntityNameFromDmlObjects = buildIdPrefixToEntityNameFromDmlObjects;
 ...
 * @param primaryKeys
 */
function defineJoinerConfig(serviceName, { alias, schema, models, linkableKeys, primaryKeys, idPrefixToEntityName, } = {}) {
 ...
 * @param models
 */
function buildIdPrefixToEntityNameFromDmlObjects(models) {
    return models.reduce((acc, model) => {
        const id = model.parse().schema.id;
        if (primary_key_1.PrimaryKeyModifier.isPrimaryKeyModifier(id) &&
            id.schema.dataType.options.prefix) {
            acc[id.schema.dataType.options.prefix] = model.name;
        }
        else if (dml_1.IdProperty.isIdProperty(id) && id.dataType.options.prefix) {
