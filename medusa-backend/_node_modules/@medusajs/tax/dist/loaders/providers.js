...
const PROVIDER_REGISTRATION_KEY = "tax_providers";
const registrationFn = async (klass, container, pluginOptions) => {
    if (!klass?.identifier) {
        throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_ARGUMENT, `Trying to register a tax provider without a provider identifier.`);
    }
    const key = `tp_${klass.identifier}${pluginOptions.id ? `_${pluginOptions.id}` : ""}`;
    container.register({
...
const registerProvidersInDb = async ({ container, }) => {
    ...
    const existingProviders = await taxProviderService.list({ id: providersToLoad }, {});
    const upsertData = [];
